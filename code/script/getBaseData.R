...
FI = log10(df_income$forestinc/2500+1)
# Get Error
...
df$ED = standardize(demographics$pres_edu)
df$K = conservation$anajuaredd+1
df$SS = standardize(as.numeric(shocks$community_help))
df$W = standardize(log1p(df_income$total_inc))
...
total_blame = c()
for(i in unique(demographics$ward)){
  total_blame = c(total_blame, sum(conservation$defor_neighbors[demographics$ward == i], na.rm = T))
}
