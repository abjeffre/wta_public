...
model{
  ...
  for ( i in 1:N ) {
      p[i] =   a + p_mu[i]*b_mu + bE[E[i]] * sigma_E + bS[S[i]]*sigma_S +  bM[M[i]]*sigma_M  + (bF_mu+bF[M[i]]*sigma_F)*FI[i] + (bW_mu+bW[M[i]]*sigma_W)*W[i] + (bFS_mu+bFS[M[i]]*sigma_FS)*FS[i]  + (bSN_mu+bSN[M[i]]*sigma_SN)*SN[i] + bRG[M[i], RG[i]] + bRL[M[i], RL[i]] +  (bEA[M[i]]*sigma_EA + bEA_mu)*sum(delta_j[M[i]][1:EA[i]]);
  }
