...
model{
  ...
  for(i in 1:2) sigma_WF[i] ~ exponential( 2 );
  for(i in 1:2) bWF[i] ~ normal( 0 , 0.5 );
  for(i in 1:2) bWF_mu[i] ~ normal( 0 , 0.5 );
  ...
  for ( i in 1:N ) {
    mu[i] =  a[1] + bE[1][E[i]] * sigma_E[1] + bS[1][S[i]]*sigma_S[1] +  bM[1][M[i]]*sigma_M[1]  + (bF_mu[1]+bF[1][M[i]]*sigma_F[1])*FI[i] + (bW_mu[1]+bW[1][M[i]]*sigma_W[1])*W[i] + (bWF_mu[1]+bWF[1][M[i]]*sigma_WF[1])*WF[i]  + (bFS_mu[1]+bFS[1][M[i]]*sigma_FS[1])*FS[i]  + (bSN_mu[1]+bSN[1][M[i]]*sigma_SN[1])*SN[i] + bRG[1][M[i], RG[i]] + bRL[1][M[i], RL[i]] + (bEA[1][M[i]]*sigma_EA[1] + bEA_mu[1])*sum(delta_j[1][M[i]][1:EA[i]]) + b_mu*mu_mu[i];
    p[i] =   a[2] + bE[2][E[i]] * sigma_E[2] + bS[2][S[i]]*sigma_S[2] +  bM[2][M[i]]*sigma_M[2]  + (bF_mu[2]+bF[2][M[i]]*sigma_F[2])*FI[i] + (bW_mu[2]+bW[2][M[i]]*sigma_W[2])*W[i] + (bWF_mu[2]+bWF[2][M[i]]*sigma_WF[2])*WF[i]  + (bFS_mu[2]+bFS[2][M[i]]*sigma_FS[2])*FS[i]  + (bSN_mu[2]+bSN[2][M[i]]*sigma_SN[2])*SN[i] + bRG[2][M[i], RG[i]] + bRL[2][M[i], RL[i]] + (bEA[2][M[i]]*sigma_EA[2] + bEA_mu[2])*sum(delta_j[2][M[i]][1:EA[i]]);
  }
